<template>
  <card class="card-product-groups">
    ...
    <card-header slot="card-header" :title="'分组列表'">
      <btn :icon="'i-plus'" :text="'新增分组88'" :mood="'emphasis'"
           @click.native="$router.push({ name: 'operation_product_group_detail', params: { group_id: 'new', tab: 'base' } })"></btn>
    </card-header>
    ...
    <table-grid-2 :data_source="product_groups" :page_num="cur_page_num"
      ...
      <table-grid-row v-for="group in product_groups">
        ...
        <table-grid-col :col_title="'含商品数'" :span="'50px'">{{ group.product_num }}</table-grid-col>
    ...
    <cheat-sheet v-if="$store.state.is_cheat_sheet_show"
      ...
      <card-product-group-containers v-if="cur_group.status == 1"
                                     :group="cur_group"
                                     @container-removed="onContainerRemoved"></card-product-group-containers>
    </cheat-sheet>
...
<script>
  ...
  export default {
    ...
    components: {
      'card-product-group-containers': require('./product_group/card_product_group_containers')
    },
    mounted () {
    ...
    methods: {
      queryProductGroups: function () {
        axios.get('/chef/api/operation/productGroup/groupList', {
          params: {
          ...
          this.product_groups = res.data.data.groups;
