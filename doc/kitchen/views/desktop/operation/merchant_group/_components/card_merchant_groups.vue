<template>
  <card class="card-merchant-groups">
    ...
    <card-header slot="card-header" :title="'分组列表'">
      <btn :icon="'i-plus'" :text="'新增分组'" :mood="'emphasis'"
           @click.native="$router.push({ name: 'operation_merchant_group_detail', params: { group_id: 'new', tab: 'base' } })"></btn>
    </card-header>
    ...
    <card-neck slot="card-neck">
      ...
      <text-input v-model="cur_search_word" :icon="'i-search'" @keyup-enter="searchMerchantGroups"></text-input>
    </card-neck>

    <table-grid-2 :data_source="merchant_groups" :page_num="cur_page_num"
                  :transition_key="cur_page_num + cur_filter_city + cur_filter_status">
      <table-grid-row v-for="group in merchant_groups">
        ...
        <table-grid-col :col_title="'运营城市'" :span="'150px'"><span v-for="city in group.location_labels">{{ city.city_name }}、</span></table-grid-col>
        ...
        <table-grid-col :col_title="'含商户数'" :span="'50px'">{{ group.merchant_num }}</table-grid-col>
        <table-grid-col :col_title="'操作'" :span="'170px'" :align="'right'">
          ...
          <btn :icon="'i-pencil'" :text="'详情'" @click.native="goMerchantGroupDetail(group)"></btn>
        </table-grid-col>
      </table-grid-row>
    </table-grid-2>
    ...
    <cheat-sheet v-if="$store.state.is_cheat_sheet_show"
      ...
      <card-merchant-group-containers v-if="cur_group.status == 1"
                                           :group="cur_group"
                                           @container-removed="onContainerRemoved"></card-merchant-group-containers>
    </cheat-sheet>
...
<script>
  ...
  export default {
    ...
    components: {
      'card-merchant-group-containers': require('./card_merchant_group_containers')
    },
    mounted () {
      this.queryMerchantGroups();
      this.$root.$emit('init-paper', '商户分组列表');
    },
    methods: {
      queryMerchantGroups: function () {
        axios.get('/chef/api/operation/merchantGroup/groupMerchantList', {
          params: {
          ...
          this.merchant_groups = res.data.data.groups;
          this.total_groups_num = parseInt(res.data.data.total);
        });
      },
      searchMerchantGroups: function () {
        // 路有变化要重置paper
      ...
      goMerchantGroupDetail: function (group) {
        this.$router.push({
