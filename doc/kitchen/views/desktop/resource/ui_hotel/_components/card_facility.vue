<template>
  <card class="card-hotel-groups">
    ...
    <card-header slot="card-header" :title="'酒店设备列表'">
      <btn :icon="'i-plus'" :text="'添加设施'" :mood="'emphasis'"
           @click.native="$router.push({ name: 'resource_hotel_facility_detail', params: { group_id: 'new', tab: 'base' } })"></btn>
    </card-header>
    ...
    <table-grid-2 :data_source="hotel_groups" :page_num="cur_page_num"
      ...
      <table-grid-row v-for="group in hotel_groups">
        ...
        <table-grid-col :col_title="'是否免费'" :span="'100px'">{{ group.is_free }}</table-grid-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goProductGroupDetail: function (facility) {
        this.$router.push({
