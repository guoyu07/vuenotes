<template>
  <card class="card-option-method">
    <card-header slot="card-header" :title="'酒店设施'" :is_save_actions_show="is_form_changed"
                 @reset-form="reset" @save-form="save">
    </card-header>

    <g-row :spacing="1">
      <g-col :span="'1/1'">
        <grid-input :title="'请选择该酒店拥有的设施'" :row_num="5.3" v-if="!!data.list">
          <g-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      getMethod: function () {
        axios.get('/chef/api/resource/hotel/HotelFacility', {
          params: {
      ...
      save: function () {
        ...
        axios.put('/chef/api/resource/hotel/HotelFacilityUpdate', post_data).then(res => {
