<template>
  <card class="card-hotel-groups">
    ...
    <card-header slot="card-header" :title="'酒店服务列表'">
      <btn :icon="'i-plus'" :text="'添加服务'" :mood="'emphasis'"
           @click.native="$router.push({ name: 'resource_hotel_service_detail', params: { group_id: 'new', tab: 'base' } })"></btn>
    </card-header>
    ...
    <table-grid-2 :data_source="hotel_groups" :page_num="cur_page_num"
      ...
      <table-grid-row v-for="group in hotel_groups">
        ...
        <table-grid-col :col_title="'名称'" :span="'150px'">{{ group.title }}</table-grid-col>
        <table-grid-col :col_title="'是否免费'" :span="'100px'">{{ group._free }}</table-grid-col>
        <table-grid-col :col_title="'描述'" :span="'100px'">{{ group.descriptive }}</table-grid-col>
        <table-grid-col :col_title="'显示顺序'">{{ group.display_order }}</table-grid-col>
  ...
  export default {
    ...
    methods: {
      queryProductGroups: function () {
        axios.get('/chef/api/resource/hotel/ServiceList', {
          params: {
