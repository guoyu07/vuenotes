        ...
        <g-row>
          ...
          <g-col :span="'1/1'">
            <grid-input :title="'是否付费'" :row_num="1">
              <tab :tabs="{ '1': '免费', '0': '收费' }" :cur_id="product_group.is_free" :in_grid="true"
  ...
  export default {
    ...
    methods: {
      ...
      saveProductGroup: function () {
        if (this.$route.params.group_id === 'new') {
          axios.post('/chef/api/resource/hotel/ServiceAdd', this.product_group).then(res => {
          ...
          axios.put('/chef/api/resource/hotel/ServiceUpdate', this.product_group).then(res => {
            this.is_form_changed = false;
      ...
      getPaperTitle: function () {
        return this.$route.params.group_id === 'new' ? '添加服务' : '酒店服务';
      },
