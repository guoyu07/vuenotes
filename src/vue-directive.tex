...
ES5无法检测到新属性添加到一个对象上或者在对象中删除，Vue.js为此增加了三种方法\$add、\$set和\$delete来添加和删除属性，同时触发视图更新。
...
针对上述情况，可以使用track-by给Vue.js发出提示，例如：
...
v-model的number修饰符可以把用户的输入自动转换为Number类型（如果原值的转换结果为NaN，则返回原值）。
